...
export default function HeaderNav({ siteName }: HeaderNavProps) {
  ...
  return (
    <>
      {/* Info Bar */}
      <div className="fixed top-0 left-0 right-0 z-[60] bg-ink-soft text-sand h-10 flex items-center justify-center">
        <div className="max-w-7xl mx-auto w-full px-4 flex items-center justify-center md:justify-between">
          <div className="flex items-center gap-4 text-xs sm:text-sm">
            <span className="font-semibold tracking-wide hidden md:inline">Bali Sport &amp; Travel</span>
      ...
      <header className={`fixed left-0 right-0 z-50 transition-all duration-300 ease-in-out ${scrolled ? "top-0 pt-2 px-3 md:px-6" : "top-10 pt-3 px-4 md:px-8"}`}>
        <div className={`max-w-7xl mx-auto rounded-full flex items-center justify-between px-4 md:px-6 transition-all duration-300 ${scrolled ? "floating-header-scrolled h-14 md:h-16" : "floating-header h-14 md:h-16"}`}>
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-2">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="px-3 xl:px-4 py-2 text-sm font-medium text-brand-green rounded-lg hover:bg-brand-green/6 hover:text-accent-gold-dark transition-colors">
                {link.label}
              </Link>
            ))}
          </nav>
        ...
        <div className="fixed inset-0 z-[55] bg-sand flex flex-col animate-fade-in-up">
          ...
          <div className="flex-1 flex flex-col justify-center px-8 gap-2">
            {navLinks.map((link, idx) => (
              <Link key={link.href} href={link.href} onClick={closeMenu} className="font-display text-4xl font-bold uppercase tracking-wide text-brand-green py-3 border-b border-brand-green/10 hover:text-accent-gold-dark transition-colors" style={{ animationDelay: `${idx * 0.06}s` }}>
                {link.label}
              </Link>
            ))}
          </div>
