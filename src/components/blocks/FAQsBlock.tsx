      ...
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <div className="space-y-4">
          {items.map((item: any, i: number) => (
            <details 
              key={item.id || i}
              className="group bg-white rounded-2xl border border-gray-200/60 dark:border-gray-800 shadow-sm [&_summary::-webkit-details-marker]:hidden"
            >
              <summary className="flex items-center justify-between gap-4 p-5 font-bold text-base text-brand-green cursor-pointer select-none focus:outline-none">
              ...
              <div className="px-5 pb-5 pt-1 text-sm text-brand-green-light border-t border-brand-green/10 leading-relaxed">
                {item.answer}
              </div>
            </details>
          ))}
