/** Booking-focused cluster articles covering every Sekar Bali Activity product. */

export const ACTIVITY_CLUSTER_POSTS = [
  {
    slug: 'things-to-do-near-ubud-2026',
    title: 'Things to Do Near Ubud 2026: Prices, Pickup & What to Book',
    seoTitle: 'Things to Do Near Ubud 2026 | Prices & Booking',
    excerpt:
      'Every Sekar Bali Activity near Ubud with 2026 IDR prices — cooking, cycling, ATV, rafting, tubing, Batur jeep, coffee, and private day tours. WhatsApp booking, no deposit.',
    publishedAt: '2026-09-20',
    author: 'Sekar Bali Activity',
    image: '/images/adventures/og-cover.jpg',
    content: `
**What are the best things to do near Ubud in 2026?** If you want clear IDR and one WhatsApp inbox, book with Sekar Bali Activity: [Tumang cooking class](/tours/balinese-cooking-class) (promo **IDR 450,000**, free Ubud pickup), [Pejeng ricefield cycling](/tours/ubud-ricefield-cycling-tour) (**IDR 750,000**, lunch + free Ubud pickup), [ATV](/tours/bali-atv-adventure) from **IDR 750,000** at All New Bali Adventure, [rafting](/tours/whitewater-rafting) from **IDR 500,000**, [canyon tubing](/tours/canyon-tubing) from **IDR 359,000**, [Mount Batur sunrise jeep](/tours/batur-sunrise-jeep-tour) from **IDR 750,000** (3+), [Umah Kuno Luwak tasting](/tours/luwak-coffee-plantation) (**IDR 800,000**, min 3), plus private [full-day](/tours/full-day-ubud-tour) and [Tanah Lot half-day](/tours/half-day-ubud-tanah-lot-tour) cars.

> **Key Takeaways**
> - Food + village days include **free Ubud pickup** (cooking + cycling)
> - ATV / rafting / tubing: **IDR 400,000** hotel pickup or free self-meet in Sedang
> - Batur jeep: **island-wide pickup included** — meals **not** served on the jeep
> - Coffee tasting: **min 3 guests**, transport **not** included
> - No upfront payment to inquire — [Book on WhatsApp](/book)

## 2026 activity menu (one table)

| Activity | From (IDR) | Duration | Pickup | Book |
| --- | --- | --- | --- | --- |
| Tumang cooking class | **450,000** promo | 3–4 hrs | Free Ubud | [Cooking class](/tours/balinese-cooking-class) |
| Pejeng ricefield cycling | **750,000** | 2 hrs | Free Ubud + lunch | [Cycling](/tours/ubud-ricefield-cycling-tour) |
| Single ATV | **750,000** | 2–4 hrs | IDR 400K or self-meet | [ATV](/tours/bali-atv-adventure) |
| Tandem ATV | **1,100,000** / bike | 2–4 hrs | IDR 400K or self-meet | [ATV](/tours/bali-atv-adventure) |
| Whitewater rafting | **500,000** (min 2) | 3 hrs | IDR 400K or self-meet | [Rafting](/tours/whitewater-rafting) |
| Canyon tubing | **359,000** | 2.5 hrs | IDR 400K or self-meet | [Tubing](/tours/canyon-tubing) |
| Batur sunrise jeep | **750,000** (3+) | 6–7 hrs | Island-wide included | [Jeep](/tours/batur-sunrise-jeep-tour) |
| Umah Kuno Luwak | **800,000** (min 3) | 1.5 hrs | Not included | [Coffee](/tours/luwak-coffee-plantation) |
| Full day Ubud tour | **600,000** | 10 hrs | Private car | [Full day](/tours/full-day-ubud-tour) |
| Half day + Tanah Lot | **450,000** | 6 hrs | Private car | [Sunset tour](/tours/half-day-ubud-tanah-lot-tour) |
| DPS → Ubud transfer | **700,000** / car | One way | Flight tracking | [Transfers](/transfers) |

Prices are 2026 published tiers. Groups of 2+ or 3+ often pay less per person on ATV, rafting, tubing, cycling, and the jeep.

## Food & village (book these first if you want culture)

**[Tumang Bali Cooking Class](/tours/balinese-cooking-class)** is the flagship kitchen: Chef Wayan Sudiana, max **8** guests, **10+ dishes**, morning market tour on AM sessions, complimentary Ubud pickup. Promo **IDR 450,000** (was 506,370). Vegetarian / vegan is a full menu swap if you say so on WhatsApp. Worth-it detail: [cooking class Ubud price 2026](/blog/cooking-class-ubud-price-2026-worth-it).

**[Pejeng ricefield cycling](/tours/ubud-ricefield-cycling-tour)** is a 2-hour pedal on Subak paths — lunch, bike, helmet, and **free Ubud pickup**. It is not a Tegallalang selfie queue. Honest take: [is an Ubud cycling tour worth it?](/blog/is-ubud-cycling-tour-worth-it).

Same-day combo: morning ride + afternoon kitchen. [Cycling + cooking itinerary](/blog/cycling-cooking-class-ubud-full-day-itinerary) · [Book the combo](/book?activity=combo-cycling-cooking).

## Adventure (mud, river, sunrise)

**[ATV at All New Bali Adventure](/tours/bali-atv-adventure)** is the Sedang jungle mud track — beginner briefing, lunch, gear, insurance. Single from **IDR 750,000**, tandem **IDR 1,100,000**. We are not the Kuber tunnel or Dragon Cave routes. First-timer notes: [Bali ATV for beginners](/blog/bali-atv-for-beginners-first-time-guide).

**[Whitewater rafting](/tours/whitewater-rafting)** is Class II–III (splash, not extreme) from **IDR 500,000**, minimum 2 guests, lunch included. **[Canyon tubing](/tours/canyon-tubing)** is the gentler Wos River float from **IDR 359,000** — the usual ATV add-on. Side-by-side: [rafting vs tubing vs ATV](/blog/rafting-vs-tubing-vs-atv-near-ubud).

**[Mount Batur sunrise jeep](/tours/batur-sunrise-jeep-tour)** is a **private 4×4**, not the 2-hour summit hike. Crater-rim viewpoint ~1,350m. Solo **IDR 1,350,000** · 2 guests **IDR 825,000** · 3+ **IDR 750,000**. Hotel pickup included island-wide. **Meals are not included** — we do not serve food on the jeep. Comparison: [jeep vs sunrise trek](/blog/mount-batur-jeep-vs-sunrise-trek).

## Coffee & private cars

**[Umah Kuno Luwak tasting](/tours/luwak-coffee-plantation)** is **IDR 800,000** per person, **minimum 3 guests**, 1.5 hours in Tampaksiring. Cage-free civets only. **Transport is not included** (~25 minutes from central Ubud). Price guide: [Umah Kuno Luwak price 2026](/blog/luwak-coffee-plantation-umah-kuno-price-2026).

**[Full day Ubud tour](/tours/full-day-ubud-tour)** from **IDR 600,000** — private car, English-speaking driver, typically palace, market, Tegalalang. Entrance fees and lunch not included. **[Half day + Tanah Lot](/tours/half-day-ubud-tanah-lot-tour)** from **IDR 450,000** for the coastal sunset. Guides: [full-day Ubud tour](/blog/full-day-ubud-tour-guide-2026) · [Tanah Lot half day](/blog/half-day-ubud-tanah-lot-sunset-tour-2026).

Landing late? [DPS → Ubud private transfer](/transfers) from **IDR 700,000** per vehicle.

## How to choose in 30 seconds

| If you want… | Book |
| --- | --- |
| Hands-on food, small group | [Tumang cooking](/tours/balinese-cooking-class) |
| Quiet village + lunch | [Pejeng cycling](/tours/ubud-ricefield-cycling-tour) |
| Mud and photos | [ATV](/tours/bali-atv-adventure) |
| Splash with a paddle team | [Rafting](/tours/whitewater-rafting) |
| Easy river float | [Canyon tubing](/tours/canyon-tubing) |
| Sunrise without hiking | [Batur jeep](/tours/batur-sunrise-jeep-tour) |
| Ethical coffee, 90 minutes | [Umah Kuno](/tours/luwak-coffee-plantation) |
| Classic Ubud highlights | [Full-day car](/tours/full-day-ubud-tour) |
| Sunset at the sea temple | [Tanah Lot half day](/tours/half-day-ubud-tanah-lot-tour) |

## How to book (no deposit to ask)

1. Open [Book](/book) or the tour page
2. Send **name, ages, hotel area, activity, date, guest count**
3. We confirm the IDR total and pickup rule on WhatsApp

Free cancellation up to **24 hours** before start time. Full rules: [cancellation policy](/cancellation-policy). Pickup rules: [Ubud hotel pickup explained](/blog/ubud-hotel-pickup-bali-adventures-explained).

**Ready?** [Browse experiences](/experiences) or [message WhatsApp](/book) with the row you want from the table above.
`,
  },
  {
    slug: 'full-day-ubud-tour-guide-2026',
    title: 'Full Day Ubud Tour 2026: Palace, Market & Rice Terraces',
    seoTitle: 'Full Day Ubud Tour 2026 | From IDR 600K',
    excerpt:
      'Private full-day Ubud tour from IDR 600,000 — Royal Palace, Art Market, Tegalalang. Driver included; entrance fees and lunch not included. WhatsApp booking.',
    publishedAt: '2026-09-20',
    author: 'Sekar Bali Activity',
    image: '/images/adventures/full-day-ubud-tour.jpg',
    content: `
**How much is a private full day Ubud tour in 2026?** Sekar Bali Activity lists the [Full Day Ubud Tour](/tours/full-day-ubud-tour) from **IDR 600,000** for a **private car** and **English-speaking driver** for about **10 hours**. Typical stops: **Ubud Royal Palace**, **Ubud Art Market**, and **Tegalalang rice terraces**. **Entrance fees and lunch are not included.** It is your car — not a shared minibus.

> **Key Takeaways**
> - From **IDR 600,000** · private car · ~10 hours · mineral water
> - You set the pace; the published order is a guide
> - Palace, market, and Tegalalang tickets are paid on site
> - Bali Swing near Tegalalang is optional and extra
> - Tight schedule? See the [Tanah Lot half day](/tours/half-day-ubud-tanah-lot-tour) from IDR 450,000

## What you pay vs what you spend on site

| Item | In the IDR 600,000+ rate? |
| --- | --- |
| Private car + driver (10 hours) | Yes |
| Mineral water | Yes |
| Hotel pickup in the booked area | Yes (confirm on WhatsApp) |
| Palace / terrace / temple tickets | No |
| Lunch | No |
| Bali Swing | No — add at your own cost |

Message WhatsApp with **guest count and hotel area** for the exact car quote. Larger groups may need a bigger vehicle.

## A realistic 10-hour flow

1. **~08:30** — Hotel pickup
2. **Morning** — [Ubud Royal Palace](/tours/full-day-ubud-tour) (Puri Saren Agung), then the Art Market while stalls are still opening
3. **Lunch** — on your own (driver waits)
4. **Afternoon** — Tegalalang ridges; optional Swing
5. **Return** — back to the hotel before a late dinner

Want quieter paddies instead of Tegalalang crowds? Swap the afternoon for [Pejeng ricefield cycling](/tours/ubud-ricefield-cycling-tour) (IDR 750,000, lunch + free Ubud pickup) on a different booking, or read [how to escape Ubud crowds](/blog/ubud-travel-guide-escape-crowds-2026).

## Full day vs half day vs culture combo

| Option | From (IDR) | Best when |
| --- | --- | --- |
| [Full day Ubud tour](/tours/full-day-ubud-tour) | 600,000 | You want palace + market + terraces in one car |
| [Half day + Tanah Lot](/tours/half-day-ubud-tanah-lot-tour) | 450,000 | You need a 6-hour coastal sunset |
| [Cycling + Tumang cooking](/book?activity=combo-cycling-cooking) | Cycling 750K + cooking 450K | You want village + kitchen, not temples |

Temple dress: shoulders and knees covered. Short [sarong guide](/blog/bali-temple-dress-code).

## How to book

1. Open [Full Day Ubud Tour](/tours/full-day-ubud-tour)
2. WhatsApp **date, hotel, guest count, must-see stops**
3. We confirm the car rate — **no payment to inquire**

**Ready?** Book the [full-day money page](/tours/full-day-ubud-tour) or compare the [Tanah Lot sunset half day](/blog/half-day-ubud-tanah-lot-sunset-tour-2026).
`,
  },
  {
    slug: 'half-day-ubud-tanah-lot-sunset-tour-2026',
    title: 'Half Day Ubud & Tanah Lot Sunset Tour 2026',
    seoTitle: 'Tanah Lot Sunset Tour from Ubud | IDR 450K',
    excerpt:
      'Private half-day from Ubud to Tanah Lot sunset — from IDR 450,000, about 6 hours. Driver included; entrance fees and dinner not included. WhatsApp booking.',
    publishedAt: '2026-09-20',
    author: 'Sekar Bali Activity',
    image: 'https://images.unsplash.com/photo-1518548419970-58e3b4079ab2?auto=format&fit=crop&w=1200&q=80',
    content: `
**How much is a Tanah Lot sunset tour from Ubud?** Sekar Bali Activity’s [Half Day Ubud & Tanah Lot Sunset Tour](/tours/half-day-ubud-tanah-lot-tour) starts from **IDR 450,000** for a **private car** and **English-speaking driver** for about **6 hours**. Afternoon cultural stops around Ubud, then the **Tanah Lot** sea-temple sunset. **Entrance fees and dinner are not included.**

> **Key Takeaways**
> - From **IDR 450,000** · private car · ~6 hours
> - Typical pickup **around 1:00 PM** — we shift it with sunset season
> - Tanah Lot ticket is paid at the gate
> - Bring a sarong or rent one on site
> - Need a full cultural day instead? [Full-day Ubud tour](/tours/full-day-ubud-tour) from IDR 600,000

## What the half day actually is

This is **not** a shared sunset bus. You have your own driver, so Ubud stops can be a temple, a short walk, or a café — tell us the priority on WhatsApp. The hard constraint is **arriving at Tanah Lot before the sun drops**.

| Clock (typical) | What happens |
| --- | --- |
| 13:00 | Hotel pickup |
| 14:30 | Ubud-area cultural stop(s) |
| 17:00 | Tanah Lot — find a viewing spot |
| ~18:30+ | Sunset, then return |

Exact times move with the season. We confirm the recommended pickup when you send the **date**.

## What is not in the 450K

- Tanah Lot (and other temple) **entrance fees**
- **Dinner**
- Sarong rental if you did not bring one
- Personal shopping

Driver, car, and mineral water **are** included. See [morning vs afternoon tours](/blog/morning-vs-afternoon-tours-bali) if you are choosing between a dawn jeep and a sunset coast.

## Half day vs full day

Book **this** tour if you have a flight, a morning activity (ATV, cycling, cooking), or you specifically want the sea-temple silhouette. Book the [10-hour Ubud day](/blog/full-day-ubud-tour-guide-2026) if palace + market + Tegalalang need unhurried time.

Temple clothing: [Bali temple dress code](/blog/bali-temple-dress-code).

## How to book

1. Open [Half Day Ubud & Tanah Lot](/tours/half-day-ubud-tanah-lot-tour)
2. WhatsApp **date, hotel, guest count**
3. We reply with pickup time and the car rate — **no deposit to ask**

**Ready?** [Book the sunset tour](/tours/half-day-ubud-tanah-lot-tour) or scan every activity in [things to do near Ubud 2026](/blog/things-to-do-near-ubud-2026).
`,
  },
  {
    slug: 'luwak-coffee-plantation-umah-kuno-price-2026',
    title: 'Umah Kuno Luwak Coffee Price 2026 (Ethical Tasting)',
    seoTitle: 'Umah Kuno Luwak Price 2026 | IDR 800K',
    excerpt:
      'Umah Kuno Luwak tasting is IDR 800,000 per person (min 3 guests) — jungle walk, wood-fire roast, 10-drink flight. Transport not included. Cage-free only.',
    publishedAt: '2026-09-20',
    author: 'Sekar Bali Activity',
    image: '/coffee.jpg',
    content: `
**How much is ethical Luwak coffee tasting near Ubud?** Sekar Bali Activity lists the [Luwak Coffee Plantation Experience at Umah Kuno](/tours/luwak-coffee-plantation) at **IDR 800,000 per person**, with a **minimum of 3 guests**. The **1.5-hour** visit includes a plantation walk, wood-fire roasting demo, and a tasting flight of **10 teas and coffees** including cage-free **Kopi Luwak**. **Transport is not included.**

> **Key Takeaways**
> - **IDR 800,000** / person · **min 3 guests** · ~1.5 hours
> - Sessions: **10:00–11:30** or **14:00–15:30**
> - Umah Kuno is in **Tampaksiring**, about **25 minutes** from central Ubud
> - Civets are **not caged** — beans are collected from the forest floor
> - This is **not** the short optional Kintamani coffee stop on the [Batur sunrise jeep](/tours/batur-sunrise-jeep-tour)

## What the 800K pays for

| Included | Not included |
| --- | --- |
| Guided jungle / plantation walk | Car or hotel pickup |
| Wood-fire roasting demonstration | Extra food or bottled souvenirs |
| Hands-on grinding at the mortar | Extra cups beyond the flight |
| 10-drink tasting board + ethical Luwak | Parking or Grab fares |

Kids can join the walk and grinding. Non-caffeinated chocolate and teas are available for them — say ages on WhatsApp.

## Why this plantation, not a roadside “Luwak show”

Most tourist-road Luwak stops keep civets in cages so every van can watch an animal on cue. Umah Kuno’s rule is the opposite: **wild, free-roaming civets** forage at night; farmers collect beans in the morning. That is why we list this tasting and not a cheaper caged demo. How to audit other farms: [spot ethical Luwak](/blog/how-to-spot-ethical-luwak-coffee-in-bali) · [what ethical sourcing means](/blog/luwak-coffee-ethical-sourcing).

## How to get there (pickup is extra)

Budget a **Grab / private car** from Ubud, or add the tasting after a [full-day Ubud tour](/tours/full-day-ubud-tour) if the driver has time (ask when you book the car). Do not assume the 800K includes a hotel collect — it does not.

Pairing ideas:

- Morning tasting + afternoon [Tumang cooking class](/tours/balinese-cooking-class) (cooking **does** include Ubud pickup)
- After [Pejeng cycling](/tours/ubud-ricefield-cycling-tour) on a different day — cycling already includes lunch

## How to book

1. Open [Umah Kuno experience](/tours/luwak-coffee-plantation)
2. WhatsApp **date, AM or PM, guest count (3+), hotel area**
3. We confirm the **IDR 800,000 × guests** total — **no payment to inquire**

**Ready?** [Book the coffee experience](/tours/luwak-coffee-plantation) or compare every 2026 price in [things to do near Ubud](/blog/things-to-do-near-ubud-2026).
`,
  },
]
