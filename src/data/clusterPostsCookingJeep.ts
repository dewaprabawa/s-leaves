/** P0 cooking + jeep cluster spokes that money pages already link to. */
export const COOKING_JEEP_CLUSTER_POSTS = [
  {
    slug: 'cooking-class-ubud-price-2026-worth-it',
    title: 'Cooking Class Ubud Price 2026: Is Tumang Worth It?',
    excerpt:
      'Tumang Bali Cooking Class is promo IDR 450,000 (was 506,370) with free Ubud pickup, max 8 guests, and 10+ dishes — honest 2026 worth-it verdict.',
    publishedAt: '2026-09-19',
    author: 'Sekar Bali Activity',
    image: '/images/cooking/buffet-spread.jpg',
    content: `
**Is a cooking class in Ubud worth it in 2026?** Yes — if you want a capped, hands-on village kitchen rather than a hotel demo. [Tumang Bali Cooking Class](/tours/balinese-cooking-class) listed by Sekar Bali Activity is promo **IDR 450,000 per person** (was IDR 506,370) for the shared class: **10+ dishes**, **max 8 guests**, Chef **Wayan Sudiana**, English instruction, and **complimentary Ubud-area hotel pickup**. Private kitchen is **IDR 633,090** (1 guest) or **IDR 1,266,180** (2 guests).

> **Key Takeaways**
> - Shared promo **IDR 450,000** / person · was **IDR 506,370** · max **8**
> - Free Ubud pickup is included — Canggu / Seminyak is a quote, not free
> - Morning class includes the pasar (market) tour; afternoon is kitchen + rice-field walk
> - Vegetarian / vegan menus are a full swap if you request them on WhatsApp
> - Worth it when you care about cooking at your own station, not watching a chef on a stage

## What you actually pay

| Option | Price (IDR) | What’s in the rate |
| --- | --- | --- |
| Shared morning or afternoon | **450,000** promo | 10+ dishes, max 8, Ubud pickup, rice-field walk; market tour on AM only |
| Private (1 guest) | **633,090** | Exclusive kitchen + same Ubud pickup rule |
| Private (2 guests) | **1,266,180** total | Exclusive kitchen for the couple |

Sekar Bali Activity books Tumang on one WhatsApp thread with cycling and other Ubud days. No deposit is required to inquire.

## When it is worth the price

Book this class if you want:

- A **named chef** and a **TripAdvisor Traveler’s Choice 2026** kitchen (5.0 from 1500+ reviews on the operator listing)
- Everyone cooking — not a 20-person demo
- Pickup from an Ubud hotel without a hidden transfer invoice
- A meal you cooked (vegetarian / vegan on request)

Skip it if you only want a cheap tasting plate with no cooking, or you need hotel pickup from Canggu included in the 450K rate — that transfer is extra.

## How it compares to a “cheap Ubud class”

Large hotel or restaurant classes often advertise a lower sticker and then add pickup, or they put 16–30 guests around one stove. Tumang’s promo is not the cheapest number in Ubud. It is the clearer number: **group cap, pickup rule, and dish count** are published before you WhatsApp.

Pair the afternoon class with [Pejeng ricefield cycling](/tours/ubud-ricefield-cycling-tour) (IDR 750,000, lunch + free Ubud pickup) for a culture day — [full itinerary](/blog/cycling-cooking-class-ubud-full-day-itinerary).

## Morning vs afternoon (price is the same)

Same promo IDR. Morning adds the traditional market tour. Afternoon skips the pasar and suits a cycling-first day. Detail: [morning vs afternoon cooking class](/blog/morning-vs-afternoon-ubud-cooking-class).

## Dietary needs

A full vegetarian or vegan menu is available — request it when you book, not at the mortar. Detail: [vegetarian Tumang menu](/blog/vegetarian-vegan-cooking-class-ubud).

## How to book

1. Open [Tumang Bali Cooking Class](/tours/balinese-cooking-class)
2. WhatsApp **name, hotel area, date, guest count, AM or PM, diet**
3. We confirm the promo or private rate — no payment to inquire

**Ready?** Book the [cooking class money page](/tours/balinese-cooking-class) or read [inside the class](/blog/inside-balinese-cooking-class-pejeng).
`,
  },
  {
    slug: 'vegetarian-vegan-cooking-class-ubud',
    title: 'Vegetarian & Vegan Cooking Class in Ubud (Tumang)',
    excerpt:
      'Tumang Bali Cooking Class offers a full vegetarian or vegan menu on request — same promo IDR 450,000, max 8, free Ubud pickup. Tell WhatsApp before class.',
    publishedAt: '2026-09-19',
    author: 'Sekar Bali Activity',
    image: '/images/cooking/dish.jpg',
    content: `
**Is there a vegetarian cooking class in Ubud?** Yes. [Tumang Bali Cooking Class](/tours/balinese-cooking-class) prepares a **full vegetarian or vegan menu** when you request it on WhatsApp — not a single tofu side next to a meat class. Shared promo is **IDR 450,000 per person** (was IDR 506,370), **max 8 guests**, complimentary **Ubud-area pickup**.

> **Key Takeaways**
> - Vegetarian and vegan are a **menu swap**, requested at booking
> - Same promo IDR and group cap as the standard shared class
> - Base Genep (spice paste) is built plant-based from the start when you tell us
> - Morning market still runs on AM sessions; afternoon is kitchen + rice fields
> - Do not wait until you arrive at the station to mention a diet

## What “full menu” means

Tumang is a family kitchen near Ubud taught by Chef **Wayan Sudiana** in English. On a standard day you may cook sate lilit, pepes, lawar, sambal matah, and more. On a vegetarian / vegan booking the kitchen **does not** leave you watching meat dishes. They plan plant-based versions before you pound spices.

Tell WhatsApp:

- Vegetarian (eggs / dairy OK or not)
- Vegan (no animal products)
- Allergies (peanut, gluten, chili heat)

## What is still included

- Hands-on cooking of **10+ dishes** at your station
- **Rice-field walk**
- **Morning market tour** on the AM shared class
- Meal of the dishes you cook
- Complimentary hotel pickup **in the Ubud area**

Pickup from Canggu, Seminyak, or south Bali is a transfer quote — same rule as the meat menu.

## Price (2026)

| Option | IDR |
| --- | --- |
| Shared (veg or standard) | **450,000** promo / person |
| Private 1 guest | **633,090** |
| Private 2 guests | **1,266,180** total |

Worth-it context: [Cooking class Ubud price 2026](/blog/cooking-class-ubud-price-2026-worth-it).

## Who this is for

Couples and small groups who want Balinese technique (stone mortar, sambal, coconut) without fish or meat. It is not a raw-vegan retreat and not a hotel buffet class.

**Ready?** Message WhatsApp from the [Tumang cooking class page](/tours/balinese-cooking-class) with **“vegetarian” or “vegan”** in the first note.
`,
  },
  {
    slug: 'morning-vs-afternoon-ubud-cooking-class',
    title: 'Morning vs Afternoon Cooking Class in Ubud',
    excerpt:
      'Tumang AM class includes the market tour; afternoon is rice-field walk + kitchen — same promo IDR 450,000 and free Ubud pickup. How to choose.',
    publishedAt: '2026-09-19',
    author: 'Sekar Bali Activity',
    image: '/images/cooking/market-guide.jpg',
    content: `
**Should you book the morning or afternoon cooking class in Ubud?** Book **morning** if you want the traditional **pasar (market) tour** plus kitchen. Book **afternoon** if you are pairing with [Pejeng ricefield cycling](/tours/ubud-ricefield-cycling-tour) earlier the same day. [Tumang Bali Cooking Class](/tours/balinese-cooking-class) charges the **same promo IDR 450,000** (was 506,370) for both shared sessions. Max **8** guests. Complimentary **Ubud pickup** on both.

> **Key Takeaways**
> - Price does not change between AM and PM
> - **Morning = market + kitchen**; **afternoon = rice fields + kitchen**
> - Both include 10+ dishes, English instruction, and the meal you cook
> - Afternoon is the usual half of a [cycling + cooking day](/blog/cycling-cooking-class-ubud-full-day-itinerary)
> - Private kitchen can sit on either clock — ask WhatsApp

## Side-by-side

| | Morning shared | Afternoon shared |
| --- | --- | --- |
| Promo price | IDR 450,000 / person | IDR 450,000 / person |
| Market tour | Yes | No |
| Rice-field walk | Yes | Yes |
| Dishes | 10+ hands-on | 10+ hands-on |
| Best after | A quiet hotel breakfast | Pejeng cycling + lunch |
| Pickup | Free in Ubud | Free in Ubud |

## Who should take morning

Travelers who want to smell galangal and turmeric in the pasar before they cook Base Genep. If the market is the reason you searched “cooking class Ubud,” take AM.

## Who should take afternoon

Guests already on a village bike day. Cycling includes lunch; you freshen up, then cook. Do not book two morning products that both want an 08:30 start.

## Vegetarian note

Diet does not lock you to one session. Request veg / vegan on either clock — [vegetarian class guide](/blog/vegetarian-vegan-cooking-class-ubud).

**Ready?** Send **AM or PM + hotel + date** on WhatsApp from the [cooking class page](/tours/balinese-cooking-class).
`,
  },
  {
    slug: 'mount-batur-sunrise-jeep-tour-guide-2026',
    title: 'Mount Batur Sunrise Jeep Tour Guide (2026)',
    excerpt:
      'No-hike 4×4 to the Mount Batur crater rim — 6–7 hours, island-wide pickup, IDR tiers, meals not included. 2026 itinerary from Ubud and south Bali.',
    publishedAt: '2026-09-19',
    author: 'Sekar Bali Activity',
    image: 'https://images.unsplash.com/photo-1693821876313-dc573a92028c?auto=format&fit=crop&w=1200&q=80',
    content: `
**What is the Mount Batur Sunrise Jeep Tour?** It is a **private 4×4** to a **crater-rim viewpoint (~1,350m)** on Mount Batur’s eastern flank near Kintamani — sunrise over **Lake Batur** and **Mount Agung** without the ~2-hour summit trek. [Sekar Bali Activity](/tours/batur-sunrise-jeep-tour) includes the jeep, local driver, **island-wide hotel pickup**, a **hot drink**, Kintamani entrance fee, and insurance for ages 6–65. **Meals are not included. Food is not served on the jeep.**

> **Key Takeaways**
> - Door to door **about 6–7 hours** · pickup typically **02:00–03:00**
> - **IDR 1,350,000** solo · **825,000** pp (2) · **750,000** pp (3+)
> - You **stay in the jeep** — not the summit hike
> - Bring a **snack and a warm layer**; we do not serve breakfast on the vehicle
> - Optional Kintamani coffee stop on the way back ≠ [Umah Kuno luwak](/tours/luwak-coffee-plantation)

## 2026 prices

| Guests in one jeep | Price per person (IDR) |
| --- | --- |
| 1 | 1,350,000 |
| 2 | 825,000 |
| 3+ | 750,000 |

Full table: [jeep price guide](/blog/mount-batur-sunrise-jeep-tour-price-guide-2026).

## Sample morning

| Time | What happens |
| --- | --- |
| 02:00–03:00 | Hotel pickup (south Bali earliest; Ubud later) |
| ~04:00 | Transfer into the 4×4 at the Kintamani base |
| ~05:45 | Arrive crater-rim viewpoint |
| ~06:00 | Sunrise — stay with the view; eat only what you brought |
| ~06:45 | Descend the volcanic tracks |
| ~08:00 | Optional coffee plantation (no obligation to buy) |
| ~09:30 | Hotel drop-off |

Pickup-by-area detail: [jeep pickup times from Canggu, Seminyak & Ubud](/blog/mount-batur-jeep-pickup-times-canggu-ubud-2026).

## Jeep vs trek

Different products. The jeep is **not** a shortcut to the summit pin. Comparison: [Mount Batur jeep vs sunrise trek](/blog/mount-batur-jeep-vs-sunrise-trek).

## What to bring

Warm jacket, closed shoes, camera, cash for optional coffee souvenirs, and **food if you want to eat**. A hot drink is included. Meals are not.

**Ready?** Book the [Mount Batur Sunrise Jeep Tour](/tours/batur-sunrise-jeep-tour) with hotel area, date, and guest count — no payment to inquire.
`,
  },
  {
    slug: 'mount-batur-jeep-vs-sunrise-trek',
    title: 'Mount Batur Jeep vs Sunrise Trek',
    excerpt:
      'Private 4×4 crater-rim viewpoint vs the 2-hour summit hike — who should skip the trek. Jeep meals are not included.',
    publishedAt: '2026-09-19',
    author: 'Sekar Bali Activity',
    image: 'https://images.unsplash.com/photo-1727335333476-8aa180978ff6?auto=format&fit=crop&w=1200&q=80',
    content: `
**Should you do a Mount Batur sunrise jeep or the summit trek?** Choose the **jeep** if you want sunrise over Lake Batur and Mount Agung **without hiking**. [Sekar Bali Activity](/tours/batur-sunrise-jeep-tour) drives a private 4×4 on volcanic tracks to an **eastern-flank crater-rim viewpoint (~1,350m)** near Kintamani. Choose a **trek** if walking the dark lava trail to the **summit** is the goal. They are different products — the jeep is not a shortcut to the same summit pin.

> **Key Takeaways**
> - Jeep: sit in a private 4×4 · no 2-hour hike · **meals not included**
> - Trek: ~2 hours up in the dark · summit trail · higher fitness
> - Jeep prices (2026): IDR **1,350,000** solo · **825,000** (2 pax) · **750,000** (3+)
> - Hotel pickup **included island-wide** on our jeep
> - Do not book the jeep if your checklist says “stand on the summit”

## Side-by-side

| | Sunrise jeep (Sekar Bali) | Classic Batur sunrise trek |
| --- | --- | --- |
| How you go up | Private 4×4 on volcanic tracks | Walk ~2 hours in the dark |
| Where you watch sunrise | Eastern-flank crater-rim viewpoint (~1,350m) | Summit trail (different route) |
| Fitness | Sit in the jeep | Moderate–hard hiking |
| Start | Hotel pickup ~02:00–03:00 | Usually similar pre-dawn start |
| Food | **Not included** — no meals on the jeep | Often a simple trek breakfast |
| Best for | Families, couples, non-hikers | Fit hikers who want the summit |
| Honest limit | Not the summit | Steeper, darker, more effort |

## Price: what the jeep includes

- 1 guest: **IDR 1,350,000**
- 2 guests: **IDR 825,000** each
- 3+ guests: **IDR 750,000** each

Included: 4×4, local driver, hotel pickup & drop-off, hot drink, Kintamani / Batur area entrance fee, insurance for ages 6–65. **Not included: breakfast or other meals.** Optional coffee plantation stop on the way back (no purchase required) — not the [Umah Kuno luwak tasting](/tours/luwak-coffee-plantation).

Trek prices vary by group size and whether a guide, headlamp, and pickup are extras.

## Pickup from Ubud vs south Bali

Our jeep includes hotel pickup island-wide. South Bali (Nusa Dua, Jimbaran, Kuta, Sanur, Seminyak, Canggu) leaves earliest; Ubud is a little later. Confirm the exact time on WhatsApp. That is different from ATV/rafting, which add IDR 400,000 for hotel pickup — see [Ubud hotel pickup explained](/blog/ubud-hotel-pickup-bali-adventures-explained) and [pickup times by area](/blog/mount-batur-jeep-pickup-times-canggu-ubud-2026).

## Who should book the jeep

Book the [Mount Batur Sunrise Jeep Tour](/tours/batur-sunrise-jeep-tour) if you want caldera sunrise photographs without scrambling rock, a private vehicle, and a seated climb.

Do not book the jeep if you need the summit stamp. We will not pretend the 4×4 replaces that hike.

Itinerary: [2026 jeep guide](/blog/mount-batur-sunrise-jeep-tour-guide-2026). Prices: [2026 price guide](/blog/mount-batur-sunrise-jeep-tour-price-guide-2026).

**Ready for sunrise without the hike?** Message WhatsApp with hotel area, date, and guest count.
`,
  },
  {
    slug: 'mount-batur-jeep-pickup-times-canggu-ubud-2026',
    title: 'Mount Batur Jeep Pickup Times from Canggu, Seminyak & Ubud',
    excerpt:
      'Island-wide hotel pickup is included on the Batur sunrise jeep — typical 02:00–03:00. South Bali leaves earliest; Ubud a little later. Meals not included.',
    publishedAt: '2026-09-19',
    author: 'Sekar Bali Activity',
    image: 'https://images.unsplash.com/photo-1725946687006-e5cf87668fd9?auto=format&fit=crop&w=1200&q=80',
    content: `
**What time is hotel pickup for the Mount Batur sunrise jeep?** Typically **02:00–03:00 AM**, and **pickup is included island-wide** in the jeep price — not the IDR 400,000 ATV/rafting add-on. [Sekar Bali Activity](/tours/batur-sunrise-jeep-tour) leaves **south Bali earliest** (Nusa Dua, Jimbaran, Kuta, Sanur, Seminyak, Canggu) and collects **Ubud a little later**. We confirm the exact window on WhatsApp after you send hotel area, date, and guest count.

> **Key Takeaways**
> - Pickup **included** from Ubud and south Bali — say the hotel name, not “near Ubud”
> - South Bali: plan to be in the lobby closer to **02:00**
> - Ubud: often closer to **02:30–03:00**
> - Door-to-door **6–7 hours**, drop-off ~09:30
> - **Meals not included** — no food served on the jeep; bring a snack

## Why the clock moves

The viewpoint is near **Kintamani / Mount Batur**. Driving time from Nusa Dua is longer than from central Ubud, so the same sunrise forces an earlier lobby call in the south.

We do not publish a fake pinned minute for every villa. The honest rule: **south earliest, Ubud later**, confirmed the day before.

## What the rate already includes

Private 4×4, driver, hotel pickup & drop-off, hot drink, entrance fee, insurance ages 6–65. **Food / breakfast is not included.**

Tiers: solo **IDR 1,350,000** · 2 guests **IDR 825,000** · 3+ **IDR 750,000** per person.

## Jeep vs ATV pickup

ATV, rafting, and canyon tubing add **IDR 400,000** for hotel pickup or you self-meet at All New Bali Adventure. The jeep is the opposite: **island-wide pickup is in the price**. Policy: [Ubud hotel pickup explained](/blog/ubud-hotel-pickup-bali-adventures-explained).

## How to book the exact time

1. Open the [jeep money page](/tours/batur-sunrise-jeep-tour)
2. WhatsApp **hotel or area + date + guest count**
3. We reply with the pickup window — no payment to inquire

Guides: [2026 itinerary](/blog/mount-batur-sunrise-jeep-tour-guide-2026) · [jeep vs trek](/blog/mount-batur-jeep-vs-sunrise-trek) · [price table](/blog/mount-batur-sunrise-jeep-tour-price-guide-2026).
`,
  },
]
